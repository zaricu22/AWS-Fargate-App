...
/**
 * AWS: Cognito access tokens carry a `client_id` claim instead of a standard
 * `aud` claim, so Spring's default issuer-uri validation (iss/exp/nbf only)
 * never checks that a token was actually issued for THIS app client.
 */
